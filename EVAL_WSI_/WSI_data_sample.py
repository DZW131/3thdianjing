...
def split_patches(image, mask,save_dir,label_count_dict,save_ind):
    ...
    for x in range(0, h-patch_size+1, patch_size-h_overlap):
        for y in range(0, w-patch_size+1, patch_size-w_overlap):
                ...
                if np.sum(patch_label[:,:,0:-1])<2000:
                    continue
                data_dict = {"img": patch, "mask": patch_label, "label_count": label_count_dict}
                patch_and_label=0.9*patch
                if f_index%4==0:
                    dd.io.save(
                    os.path.join(save_dir, "val","h5","%s_%d.h5" % (save_ind, f_index)), data_dict)

                    for i in range(4):
                        cur_label=patch_label[:,:,i]//255
                        cur_label=cur_label[:,:,np.newaxis]
                        cur_label=np.repeat(cur_label,3,-1)
                        cur_color=list(label_color.values())[i]
                        label_region_color=cur_label*cur_color
                        patch_and_label=patch_and_label+0.1*label_region_color
                    cv2.imwrite(os.path.join(save_dir, "val", "images", "%s_%d.jpg" % (save_ind, f_index)), patch)
                    cv2.imwrite(os.path.join(save_dir, "val","labels","%s_%d.jpg" % (save_ind, f_index)),patch_and_label)
                    # cv2.imwrite(os.path.join(save_dir, "val", "labels", "%s_%d.jpg" % (save_ind, f_index)), patch_label)
                else:
                    dd.io.save(
                        os.path.join(save_dir, "train", "h5", "%s_%d.h5" % (save_ind, f_index)), data_dict)
                    for i in range(4):
                        cur_label = patch_label[:, :, i]//255
                        cur_label = cur_label[:, :, np.newaxis]
                        cur_label = np.repeat(cur_label, 3, -1)
                        ...
                        label_region_color = cur_label * cur_color
                        patch_and_label = patch_and_label + 0.1 * label_region_color
                    # cv2.imwrite(os.path.join(save_dir, "train", "labels", "%s_%d.jpg" % (save_ind, f_index)),
                    #             patch_and_label)
                    cv2.imwrite(os.path.join(save_dir, "train", "images", "%s_%d.jpg" % (save_ind, f_index)), patch)
                    cv2.imwrite(os.path.join(save_dir, "train", "labels", "%s_%d.jpg" % (save_ind, f_index)), patch_and_label)
                f_index+=1
...
def vis_anno(slide_path,roi_contours, roi_labels,level,save_dir,index):
    ...
    coords_np_list = []
    label_color=[(255, 0, 0),(0, 255, 0),(0, 0, 255),(128, 0, 255),(128, 0, 128),(0, 255, 255)]
    for i, roi_contour in enumerate(roi_contours):
        ...
        cur_mask_fortrain = cv2.drawContours(cur_mask_fortrain, coords_np_list, -1, (255,0,0), cv2.FILLED)
    ...
    thumbnail_with_mask = cv2.addWeighted(mask_all, 0.2, thumbnail, 0.8, 0)
    # thumbnail_with_mask = cv2.cvtColor(thumbnail_with_mask, cv2.COLOR_BGR2RGB)
    cv2.imwrite('../region_test_data/%s.png'%slide_name, thumbnail_with_mask)
...
if __name__ == '__main__':
    ...
    mrxs_img_folder = '/media/deepin/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/3.28区域标注json'
    # label_dict = {"筛状": 0, '粉刺型': 1, '实性型': 2, '淋巴组织': 3, '脂肪': 4, '筛状型': 0,'淋巴细胞':3}
    # label_dict = {"筛状": 0, '粉刺型': 1, '实性型': 2,'脂肪': 3, '筛状型': 0}
    label_dict = {"筛状": 1, '粉刺型': 2, '实性型': 3, '筛状型': 1}
    label_color = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (128, 0, 255), (128, 0, 128), (255, 255, 255)]
    label_card=[]
    for cor_ind in range(5):
        this_card = np.ones((100, 200, 3))
        this_card = this_card * np.array(label_color[cor_ind])[np.newaxis, np.newaxis, :]
        label_card.append(this_card)
    label_card = np.concatenate(label_card, axis=0).astype(np.float32)
    label_card=label_card[:,:,::-1]
