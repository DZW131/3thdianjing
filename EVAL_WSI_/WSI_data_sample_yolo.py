...
def split_patches(image, yolo_anno_np_list, slide_path, save_dir, label_count_dict, save_ind,):
    ...
    f_index = 0
    #split data
    for anno in yolo_anno_np_list:
        if anno["is_display"] == True:
            continue
        patch_x = int(anno["x_center"]-patch_size/2)
        patch_y = int(anno["y_center"]-patch_size/2)
        patch_x = patch_x if patch_x >= 0 else 0
        patch_y = patch_y if patch_y >= 0 else 0
        patch_x = patch_x if patch_x+patch_size <= w else w-patch_size
        patch_y = patch_y if patch_y+patch_size <= h else h-patch_size

        patch = image[patch_y:patch_y+patch_size, patch_x:patch_x+patch_size, :]
        print('extract %s in %d WSI' % (get_keys(label_dict, anno["label"])[0], save_ind))

        if f_index%4==0:
            cv2.imwrite(os.path.join(save_dir, 'images', 'val', "%d_%d_" % (save_ind, f_index) + fname + ".jpg"), patch)
        else:
            cv2.imwrite(os.path.join(save_dir, 'images', 'train', "%d_%d_" % (save_ind, f_index) + fname + ".jpg"), patch)

        file_data = ""
        for i in range(len(yolo_anno_np_list)):
            # if anno["is_display"] == True:
            #     continue
            if yolo_anno_np_list[i]["x_center"] - yolo_anno_np_list[i]["w"] / 2 >= patch_x and \
               yolo_anno_np_list[i]["x_center"] + yolo_anno_np_list[i]["w"] / 2 <= patch_x + patch_size and \
               yolo_anno_np_list[i]["y_center"] - yolo_anno_np_list[i]["h"] / 2 >= patch_y and \
               yolo_anno_np_list[i]["y_center"] + yolo_anno_np_list[i]["h"] / 2 <= patch_y + patch_size:
                file_data += str(yolo_anno_np_list[i]["label"]) + \
                             ' ' + '%.6f' % ((yolo_anno_np_list[i]["x_center"]-patch_x) / patch_size) + \
                             ' ' + '%.6f' % ((yolo_anno_np_list[i]["y_center"]-patch_y)/ patch_size) + \
                             ' ' + '%.6f' % (yolo_anno_np_list[i]["w"] / patch_size) + \
                             ' ' + '%.6f' % (yolo_anno_np_list[i]["h"] / patch_size) + '\n'
                yolo_anno_np_list[i]["is_display"] = True

        if f_index%4==0:
            with open(os.path.join(save_dir, 'labels', 'val', "%d_%d_" % (save_ind, f_index) + fname + ".txt"), 'a') as f:
                f.write(file_data)
        else:
            with open(os.path.join(save_dir, 'labels', 'train', "%d_%d_" % (save_ind, f_index) + fname + ".txt"), 'a') as f:
                f.write(file_data)
        f_index += 1
...
def vis_anno(slide_path,roi_contours, roi_labels,level,save_dir,index):
    ...
    if 'ndpi' in slide_path or 'mrxs' in slide_path or 'sdpc' or 'kfb' in slide_path:
        ...
        slide = openSlide(slide_path)
        thumbnail = slide.read(slide,location=(0, 0), level = level, size = dim)
    ...
    for i, roi_contour in enumerate(roi_contours):
        ...
        w = x_max-x_min
        h = y_max-y_min
        yolo_anno_np_list.append({"label": label, "x_center": x_center, "y_center": y_center, "w": w, "h": h, "is_display": False})


    split_patches(thumbnail, yolo_anno_np_list, slide_path, save_dir=save_dir, save_ind=index, label_count_dict=label_dict)
...
if __name__ == '__main__':
    ...
    save_dir='./data/yolo_Her2Data_2'
    img_folder = '/media/zhouzhihao/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/region_data/测试用切片'
    # img_folder = '/media/zhouzhihao/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/region_data/测试用切片/上肿总切片/3月切片_第二批'
    # label_dict = {"筛状": 0, '粉刺型': 1, '实性型': 2, '淋巴组织': 3, '脂肪': 4, '筛状型': 0,'淋巴细胞':3}
    # label_dict = {"筛状": 1, '粉刺型': 2, '实性型': 3, '筛状型': 1,'正常导管':4}
    label_dict = {"筛状": 0, '筛状型': 0, '粉刺型': 0, '实性型': 0} # class映射的值必须从1开始，且为连续的整数
    label_num = 1 + 1
    level = 5
    randomrate = 0.3
