...
def split_patches(image, mask, slide_path, save_dir, label_count_dict,save_ind):
    ...
    cv2.imwrite(os.path.join(save_dir, str(save_ind) + "_" + fname + "_ori.jpg"), image)
...
def vis_anno(slide_path,roi_contours, roi_labels,level,save_dir,index):
    ...
    # slide_path = r"/media/zhouzhihao/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/region_data/训练集/1141 0.mrxs"
    if 'ndpi' in slide_path or 'mrxs' in slide_path or 'sdpc' or 'kfb' in slide_path:
        slide = openSlide(slide_path)
        thumbnail = np.array(
    ...
    for i, roi_contour in enumerate(roi_contours):
        ...
        # # 膨胀
        # single_channel = cur_mask_fortrain[:, :, 0].copy()
        # kernel = np.ones((3, 3), np.uint8)
        # single_channel = cv2.dilate(single_channel, kernel, iterations=3)
        # # single_channel = cv2.erode(single_channel, kernel, iterations=3)
        # cur_mask_fortrain[:, :, 0] = single_channel
...
if __name__ == '__main__':
    ...
    save_dir='../../yolo/data/yolo_Her2Data_6/vis_anno_segmentation'
