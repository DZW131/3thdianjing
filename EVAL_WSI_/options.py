...
class Options:
    def __init__(self, isTrain):
        ...
        self.model = dict()
        self.model['name'] = 'Resnet_Unet'
        self.model['C_name']='Resnet_Unet'
        self.model['R_name'] = 'Unet'
        self.model['in_c'] = 3       # input channel
        self.model['out_c'] = 10      # o                  output channel
        ...
        # --- training params --- #
        self.train = dict()
        self.train['data_dir'] = '../data'  # path to data
        self.train['save_dir'] = './experiments/{:s}/{:s}'.format(self.dataset,self.model['name'])
        self.train['input_size'] = 512   # input size of the image
        self.train['num_epochs'] = 500
        ...
        self.train['decay_epoch'] = 300#self.train['num_epochs']//2#learning rate decay epoch
        ...
        self.test['save_dir'] = './experiments/{:s}/{:s}/{:s}'.format(self.dataset, self.model['name'],self.test['epoch'])
        self.test['model_path'] = 'experiments/{:s}/{:s}/checkpoints/checkpoint_{:s}.pth.tar'.format(self.dataset,self.model['name'], self.test['epoch'])
        self.test['WSI_save_dir']= '/media/deepin/DeepInformatic_dataset/lihansheng/HER2/0503/{:s}/'.format('patches')
        # --- post processing --- #
        self.post = dict()
    ...
    def parse(self):
        ...
        if self.isTrain:
            ...
            #         'random_crop': self.train['input_size'],
                    #'label_encoding': 1,
                    # 'to_tensor': 1,
                    # 'normalize': np.load('mean_std.npy')
                # }
                # self.transform['val'] = {
                #     'label_encoding': 1,
                #     'to_tensor': 1,
