...
class KfbSlide():
    ...
    def read_region(self, location=[0,0], level=0, size = [256, 256] ):
        ...
        for rx in range(num_x-1): # traverse through x
            for ry in range(num_y-1): # traverse though y
                ...
                #                                      level=level)
                try:
                    cur_region = dzi_obj.get_tile(tile_index, (rx+start_x, ry+start_y))#PIL Image
                except:
