...
if __name__ == '__main__':
    ...
    mrxs_img_folder = '/media/zhouzhihao/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/region_data/测试用切片(加入训练，新加了标注)/'
    # mrxs_img_folder = '/media/zhouzhihao/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/test_3/'
    # label_dict = {"筛状": 0, '粉刺型': 1, '实性型': 2, '淋巴组织': 3, '脂肪': 4, '筛状型': 0,'淋巴细胞':3}
    # label_dict = {"筛状": 1, '粉刺型': 2, '实性型': 3, '筛状型': 1,'正常导管':4}
    label_dict = {"筛状": 1, '筛状型': 1, '粉刺型': 1, '实性型': 1, '正常导管': 2} # class映射的值必须从1开始，且为连续的整数
    # label_dict = {"筛状": 1, '粉刺型': 2, '实性型': 3, '淋巴组织': 4, '脂肪': 5, '筛状型': 1,'淋巴细胞':4,'正常导管': 6}  # class映射的值必须从1开始，且为连续的整数
    label_num = 2 + 1
    randomrate = 0.3
    if not os.path.exists(save_dir):
        ...
        if not os.path.exists(os.path.join(save_dir, 'ImageSets')):
            ...
            os.makedirs(os.path.join(save_dir, 'ImageSets'),exist_ok=True)
