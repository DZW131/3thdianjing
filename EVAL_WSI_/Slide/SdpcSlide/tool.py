...
sys.path.append(base_dir)
from EVAL_WSI_1GPU.Slide.SlideBase import SlideBase
import numpy as np
from PIL import Image
from ctypes import *

if sys.platform=='win32':
    from pystruct_win import SqSdpcInfo
    cur_encoding='gbk'
    ...
    from pystruct_linux import SqSdpcInfo
    cur_encoding='utf-8'
    lib = cdll.LoadLibrary(os.path.join(os.path.dirname(os.path.abspath(__file__)),'linux_libs','libmysdpc.so'))
...
class SdpcSlide(SlideBase):
    ...
    def read(self, location=[0, 0], size=None, scale=1, greyscale=False):
        ...
        if self.scale == 0.5:
            ...
            crop_level = math.floor(math.log(scale)/math.log(4))
        else:
            raise Exception('scale is not 0.5 or 0.25 that we need!')
        crop_level = max(0, min(crop_level, self.slide.contents.picHead.contents.hierarchy-1))

        level_ratio = (1/self.scale) ** crop_level
        ...
        # lib.SqGetRoiRgbOfSpecifyLayer(self.slide, byref(rgb), width, height, xp, yp, layer)
...
if __name__ == "__main__":
    ...
    slide = SdpcSlide(r"/media/zhouzhihao/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/需可视化切片/RJ2021102024699.sdpc")
    cur_region = np.array(slide.read((14336, 0), (1024, 1024), 1))
    cur_region = cur_region.astype(np.uint8)
    cv2.imwrite('/media/zhouzhihao/DeepInformatic_dataset/lihansheng/HER2/code/cell_infer/sdpc.jpg', cur_region)
