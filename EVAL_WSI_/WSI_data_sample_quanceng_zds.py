...
def get_contours(slide_path,level,ext=('.svs', '.sdpc')):
    dir_name = os.path.dirname(slide_path)
    for file_ext in ext:
        index_file = slide_path.replace(file_ext, '.json')
...
def vis_anno(slide_path,roi_contours, roi_labels,level,save_dir,index):
    ...
    # print(dim)
    if 'ndpi' in slide_path or 'mrxs' in slide_path  or 'svs' in slide_path:
        slide = openslide.OpenSlide(slide_path)
        dim = (slide.width, slide.height)
        thumbnail = np.array(
        ...
        thumbnail = slide.read(location=(0, 0),size=dim,scale=16)
