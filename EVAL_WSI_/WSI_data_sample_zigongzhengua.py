...
def get_contours(slide_path,level,ext='.svs'):
    ...
    for i, roi_dict in enumerate(roilist):
        ...
        try:
            ...
            continue
        try:
            remark=label_dict[remark_name[0]]
        except:
            continue
...
def vis_anno(slide_path,roi_contours, roi_labels,level,save_dir,index):
    ...
    print(level_dim_dict)
    dict_length = len(level_dim_dict)
    if dict_length<4:
        return 
    # 如果字典的长度小于4的话就跳过
    # scale = level_dim_dict[level-1][1]
    dim = level_dim_dict[level][0]
    # print(dim)
    if 'ndpi' in slide_path or 'mrxs' in slide_path or 'sdpc' in slide_path or 'svs' in slide_path:
        slide = openslide.OpenSlide(slide_path)
        thumbnail = np.array(
        ...
        thumbnail = read_region_kfb(location=(0, 0), level = level, size = dim)
    ...
    for i, roi_contour in enumerate(roi_contours):
        ...
        x_coord = np.array([int(contour[0]/ 4**level) for contour in roi_contour])
        y_coord = np.array([int(contour[1]/4**level) for contour in roi_contour])
        # print(x_coord)
...
if __name__ == '__main__':
    ...
    save_dir='/home/songlinru/Project/zds_zgjzg'
    img_folder = '/home/songlinru/data/WSI/河南省妇幼保健院_2404_子宫诊刮项目'
    ...
    for ext in ext_list:
        ...
        for ind,slide_path in tqdm(enumerate(imglist)):
            ...
            if json_name not in json_name_list:
                ...
                # mpp=0.25, level=5; mpp=0.5, level=4;
                # if (mpp > 0.45 and mpp < 0.55):
                #     level = 3
                #     mpp_dic["mpp=0.50"] += 1
                # elif (mpp > 0.2 and mpp < 0.3):
                #     level = 4
                #     mpp_dic["mpp=0.25"] += 1
                # else:
                #     level = 4
