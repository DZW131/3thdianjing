...
def split_kfb(slides_dir,result_summary, crop_len, C_net, R_net, is_display):
    ...
    if not os.path.exists(os.path.join(result_save_sub_dir, "summary_{}.csv".format(kfb_name))):
        ...
        result_masks = generate_region_mask(thumbnail, R_net, patch_size=512, overlap=0,
                                            ori_size=dim)
        result_masks_forsave=-(result_masks-1)
        mask_save=0.2*result_masks_forsave[:,:,np.newaxis]*(0,0,255)+0.8*thumbnail
        cv2.imwrite(os.path.join(result_save_sub_dir, 'region_results.jpg'), mask_save)
        del mask_save
        #for cell
                        ...
                        for roi_list in contours:
                            ...
                            if flag == 1 or flag_1 == 1 or flag_2 == 1 or flag_3 == 1:
                                ...
                                if is_display:
                                    ...
                                    a_mask=-(a_mask-1)
                                    test_res_img=0.9*test_res_img+0.1*a_mask*(0,0,255)
