                        ...
                        for j in range(num_x):
                            ...
                            for roi_list in contours:
                                ...
                                if flag == 1 or flag_1==1 or flag_2==1 or flag_3==1:
                                    ...
                                    # img = Image.fromarray(cur_region[:,:,::-1])#.convert('RGB')
...
def compare_with_annotations( test_name, net,ID_list, is_display=True):
    ...
    kfb_new_list=[]
    for ind,kfb in enumerate(kfb_list):
        flag=0
        for ID in ID_list:
            if ID in kfb:
                flag=1
        if flag==1:
            kfb_new_list.append(kfb)
    mrxs_list = filesystem.find_ext_files(test_dir, '.mrxs')
    for slide_path in kfb_new_list:
...
def run_test(test_dir, weight_path,ID_list):
    ...
    compare_with_annotations(test_name=test_name, net=net, ID_list=ID_list,is_display=True)
...
if __name__ == "__main__":
    ...
    opt.save_options()
    os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    weight_path=os.path.abspath('..')+'/'+opt.test['model_path']
    ...
    run_test(test_dir, weight_path=weight_path,ID_list=ID_list)
