...
from Core.papSmear.only_multi_cls_cell_seg import cal_ki67_np
import math
...
def view_bar(message, num, total):
    ...
    sys.stdout.write(r)
    sys.stdout.flush()
def split_kfb(slides_dir,result_summary, crop_len, net,  is_display):
    ...
    if not os.path.exists(os.path.join(result_save_sub_dir, "summary_{}.csv".format(kfb_name))):
                        ...
                        for j in range(num_x):
                            ...
                            for roi_list in contours:
                                ...
                                if flag == 1 or flag_1==1 or flag_2==1 or flag_3==1:
                                    ...
                                    cv2.imwrite('region_%d.jpg' % count, cur_region)
            ...
            for i in range(num_y):
                ...
                for j in range(num_x):
                    ...
                    if is_display:
                        ...
                        cv2.imwrite(os.path.join(result_save_sub_dir, img_name),
                                    test_res_img)
        # with open(os.path.join(result_save_sub_dir,'result.txt'),'a+') as f:
        #     f.write(kfb_name+'\t')
        #     with open(ano_txt,'r') as label_file:
        ...
        columns = ['slide name', '微弱的不完整膜阳性肿瘤细胞', '弱-中等的完整细胞膜阳性肿瘤细胞', '阴性肿瘤细胞', '纤维细胞', '淋巴细胞', '难以区分的非肿瘤细胞', '组织细胞', '强度的完整细胞膜阳性肿瘤细胞', '中-强度的不完整细胞膜阳性肿瘤细胞',
                   '细胞总数']
        df.to_csv(os.path.join(result_save_sub_dir, "summary_{}.csv".format(kfb_name)),
                  columns=columns, index=False)


def compare_with_annotations( test_name, net, is_display=True):
    ...
    mrxs_list = filesystem.find_ext_files(test_dir, '.mrxs')[::-1]
    ...
    for slide_path in kfb_list:
        ...
        split_kfb(slide_path, result_summary,1024, net, is_display)
...
def run_test(test_dir, weight_path):
    if opt.model['name'] == 'FullNet':
        ...
        net = Resnet_Unet(opt.model['in_c'], opt.model['out_c'], resnet_pretrain=False)
    ...
    compare_with_annotations(test_name=test_name, net=net,is_display=True)
...
if __name__ == "__main__":
    ...
    opt.save_options()
    os.environ["CUDA_VISIBLE_DEVICES"] = "1"
    weight_path='../weights/C_net.pth.tar'
    test_dir = "/media/deepin/DeepInformatic_dataset/lihansheng/HER2/免疫组化her2切片/免疫组化无1546"
    # run_test(test_dir, 0,
    ...
    run_test(test_dir, weight_path=weight_path)
